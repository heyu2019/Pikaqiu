...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"src/js/player.min.js":[function(require,module,exports) {
function zplayer(e) {
...
zplayer.prototype.init = function () {
  ...
  ht += '<div class="zplayer">' + '	<div class="zplayer-content">' + '		<div class="zplayer-pic">' + '			<img src="' + this.musics[0].pic + '">' + '		</div>' + '		<div class="zplayer-info">' + '			<div class="zplayer-music">' + '			    <div class="zplayer-music-text">' + '				  <span class="zplayer-title">' + this.musics[0].title + '</span>' + '				  <span class="zplayer-author"> - ' + this.musics[0].author + '</span>' + '			    </div>' + '				<i class="zplayer-list-btn z-icon z-icon-reorder">' + '				</i>' + '			</div>';

  if (!this.option.fixed) {
    ht += this.option.showLrc ? '<div class="zlrc">' : '<div class="zlrc" style="display:none">';
    ht += '	<div class="zplayer-lrc">' + '		<div class="zplayer-lrc-contents" style="transform: translateY(0);">' + '		</div>' + '	</div>' + '</div>';
  }

  ht += '			<div class="zplayer-controller">' + '				<div class="zplayer-bar-wrap">' + '					<div class="zplayer-bar">' + '						<div class="zplayer-loaded" style="width: 0"></div>' + '						<div class="zplayer-played" style="width: 0">' + '							<span class="zplayer-thumb"></span>' + '						</div>' + '					</div>' + '				</div>' + '				<div class="zplayer-time">' + '					<span class="zplayer-ptime">00:00</span>/<span class="zplayer-dtime">00:00</span>' + '					<div class="zplayer-menu-bars">' + '						<i class="z-icon z-icon-backward"></i>' + '						<i class="z-icon z-icon-play"></i>' + '						<i class="z-icon z-icon-pause display-none"></i>' + '						<i class="z-icon z-icon-forward"></i>' + '					</div>' + '					<div class="zplayer-menu-volume">';
  ...
  ht += '<i class="player-type-icon z-icon z-icon-retweet"></i>' + '					</div>' + '				</div>' + '			</div>' + '		</div>' + '	</div>';
  ...
  if (!this.isMobile) {
    this.volumeIcon.addEventListener("click", function () {
      p.audio.muted ? (p.audio.muted = !1, p.volumeIcon.className = 0.8 <= p.audio.volume ? "volume-icon z-icon z-icon-volume-up" : "volume-icon z-icon z-icon-volume-down", p.updateBar.call(p, "volumePlayed", p.audio.volume, "width")) : (p.audio.muted = !0, p.volumeIcon.className = "volume-icon z-icon z-icon-volume-off", p.updateBar.call(p, "volumePlayed", 0, "width"));
    });
  }
  ...
  function playSwitch(i) {
    ...
    p.playedTime = setInterval(function () {
      p.updateBar.call(p, "played", p.audio.currentTime / p.audio.duration, "width"), p.option.lrcStart && p.updateLrc.call(p), p.element.getElementsByClassName("zplayer-ptime")[0].innerHTML = p.secondToTime(p.audio.currentTime);
    }, 100);
  }
...
zplayer.prototype.play = function () {
  ...
  this.playedTime = setInterval(function () {
    ...
    e.option.lrcStart && e.updateLrc.call(e), e.element.getElementsByClassName("zplayer-ptime")[0].innerHTML = e.secondToTime(e.audio.currentTime);
  }, 100);
};
...
zplayer.prototype.updateLrc = function (e) {
  if (e || (e = this.audio.currentTime), e < this.lrc[this.lrcIndex][0] || (this.lrcIndex == this.lrc.length - 1 ? false : e >= this.lrc[this.lrcIndex + 1][0])) for (var a = 0; a < this.lrc.length; a++) {
    e >= this.lrc[a][0] && (!this.lrc[a + 1] || e < this.lrc[a + 1][0]) && (this.lrcIndex = a, this.lrcContents.style.transform = "translateY(" + 20 * -this.lrcIndex + "px)", this.lrcContents.getElementsByClassName("zplayer-lrc-current")[0].classList.remove("zplayer-lrc-current"), this.lrcContents.getElementsByTagName("p")[a].classList.add("zplayer-lrc-current"));
  }
};
...
},{}]},{},["C:/Users/a2289/AppData/Local/Yarn/Data/global/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","src/js/player.min.js"], null)
//# sourceMappingURL=/player.min.9c72a812.js.map